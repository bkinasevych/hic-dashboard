...
plot_theme <- function(){
  theme_minimal(base_family = "Encode Sans",
  base_size = 12) +
...
projects <- read_csv("data/tbl_projects.csv") |>
  clean_names() |> 
  select(project_id, request_date, project_name, service_type, decision, status) |> 
  ...
  mutate(service_type = case_when(
    service_type %in% c("Strategic planning", "Program evaluation") ~ service_type,
    TRUE ~ "Other"
  )) |> 
...
fees <- read_csv("data/tbl_fees.csv") |>
  ...
  mutate(fiscal_year = case_when(quarter == 4 ~ year(payment_date) - 1,
