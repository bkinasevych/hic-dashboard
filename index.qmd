...
projects <- read_csv("data/tbl_projects.csv") |>
  clean_names() |> 
  select(project_id, request_date, project_name, strategic_planning:decision) |> 
...
projects <- projects |> 
  pivot_longer(cols = c(strategic_planning:service_type_other),
               names_to = "type") |> 
  mutate(type = case_when(
    type == "consultation" ~ "other",
    type == "data_management" ~ "other",
    type == "training" ~ "other",
    type == "sroi" ~ "other",
    type == "organizational_health" ~ "other",
    type == "service_type_other" ~ "other",
    TRUE ~ type
  )) |> 
  filter(value == TRUE) |> 
  mutate(type = str_replace(type, "_", " ")) |> 
  mutate(type = str_to_title(type)) |> 
  mutate(type = factor(type, levels = c("Evaluation", "Strategic Planning", "Other"))) |> 
  select(project_id:project_name, type, decision, quarter, fiscal_year)
...
### Fees committeed per fiscal year
```{r}
fees |> 
  group_by(fiscal_year) |> 
  ...
  geom_text(aes(label = dollar(revenue)),
            vjust = -0.5) +
  labs(x = "",
       y = "") +
  plot_theme() +
