...
## Median of total MOU value by service type

```{r}
service_type <- projects |> 
  select(service_type, project_id, fiscal_year)

fees |> 
  select(project_id, payment_due) |> 
  group_by(project_id) |> 
  summarize(
  total_fees = sum(payment_due)  
  ) |> 
  left_join(service_type) |> 
  group_by(fiscal_year, service_type) |> 
  filter(service_type != "Other") |> 
  summarize(median = median(total_fees, na.rm = TRUE)) |> 
  filter(fiscal_year %in% c(2021, 2022, 2023, 2024, 2025)) |> 
  ...
  labs(x = "",
       y = "Median fees",
       fill = "",
       caption = "Fiscal Year is based on the year in which the MOU was signed") +
  plot_theme() +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor.x = element_blank()
